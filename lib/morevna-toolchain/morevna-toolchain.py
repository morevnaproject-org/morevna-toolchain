...
def exec_app(app, *args):
    ...
    MOREVNATOOLCHAIN_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            ...
            with open(config_file, "r") as f:
                for line in f:
                    if line.strip().startswith("export "):
                        ...
                        os.environ[key.strip()] = value.strip()[1:-1]
    ...
    URI_0INSTALL=""
    EXTENSIONS=[]
    if os.path.exists(os.path.join(MOREVNATOOLCHAIN_DIR, 'share', 'morevna-toolchain', 'packages', app + '.conf')):
        with open(os.path.join(MOREVNATOOLCHAIN_DIR, 'share', 'morevna-toolchain', 'packages', app + '.conf'), 'r') as f:
            for line in f:
    ...
    if app_version:
        ...
        if os.path.exists(os.path.join(MOREVNATOOLCHAIN_DIR, 'etc', 'morevna-toolchain.d', app + '.ini')):
            ...
            with open(os.path.join(MOREVNATOOLCHAIN_DIR, 'etc', 'morevna-toolchain.d', app + '.ini'), 'r') as f:
                for line in f:
                    ...
                    if len(version.strip().split("_"))>1 and version.strip().split("_")[1] == app_version:
                        app_path = path.strip()
                        break
        ...
        # 3. Use 0-install
        if found != 1 and os.environ['USE_0INSTALL'] == 1 and URI_0INSTALL != "":
...
#---------------------------------------------------------
        ...
        # 3. Use 0-install
        if found != 1 and USE_0INSTALL == 1 and 'URI_0INSTALL' in os.environ:
            version_option = ''
            found = 1
            run(['0install', 'run', version_option, f'--command={binary_0install_option}', os.environ['URI_0INSTALL']] + list(args[1:]))
        ...
        if found != 1:
            ...
            for optdir in [os.path.join(MOREVNATOOLCHAIN_DIR, 'opt'), '/opt']:
                if os.path.exists(os.path.join(optdir, app + '-*')):
                    for dir in os.listdir(os.path.join(optdir, app + '-*')):
                        app_path = os.path.join(dir, binary)
                        if os.path.exists(app_path) and os.access(app_path, os.X_OK):
                            version = dir.split('-')[-1]
                            result = compare_versions(latest_version, version)
                            if result == '<':
    ...
    if found != 1:
        ...
        print()
        print(f"              {app_name_uppercase}_DEFAULT=\"/path/to/app/binary\"")
        print()
        sys.exit(1)
